...
@objc public protocol GrowingTextViewDelegate: UITextViewDelegate {
    ...
    @objc optional func textViewDidChangeText(_ textView: GrowingTextView, text: String)
}
...
open class GrowingTextView: UITextView {
    ...
    private var heightConstraint1: NSLayoutConstraint?
    ...
    override open func layoutSubviews() {
        ...
        if (heightConstraint1 == nil) {
            heightConstraint1 = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(heightConstraint1!)
        }
        
        // Update height constraint if needed
        if height != heightConstraint1!.constant {
            shouldScrollAfterHeightChanged = true
            heightConstraint1!.constant = height
    ...
    @objc func textDidChange(notification: Notification) {
        if let sender = notification.object as? GrowingTextView, sender == self {
            ...
            setNeedsDisplay()
            if let delegate = delegate as? GrowingTextViewDelegate {
                delegate.textViewDidChangeText?(sender, text: text)
            }
        }
    }
}
