...
open class DropDown : UITextField{
    ...
    func setupUI () {
        ...
        let origin = CGPoint(x: center.x - arrowSize/2,y: center.y - arrowSize/2)
        arrow = Arrow(origin: origin,size: arrowSize)
        crossButton = getCrossButton(origin: origin)
        arrowContainerView.addSubview(arrow)
        arrowContainerView.addSubview(crossButton)
        crossButton.addTarget(self, action: #selector(didButtonClick), for: .touchUpInside)
...
extension DropDown : UITextFieldDelegate {
    ...
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        guard let index = selectedIndex else { return  isSearchEnable}
        return false
    }
...
extension DropDown: UITableViewDelegate {
    ...
    func getCrossButton(origin:CGPoint) -> UIButton{
        var crossButton  = UIButton(type: .custom)
        crossButton.frame = CGRect(x: origin.x, y: origin.y, width: arrowSize, height: arrowSize)
        crossButton.setImage(#imageLiteral(resourceName: "icDust"), for: .normal)
        crossButton.contentHorizontalAlignment = .center
        crossButton.contentVerticalAlignment = .center
        return crossButton
    }
}
